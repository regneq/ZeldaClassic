    ...
    case wFire:
    {
        ...
        if(parentitem<0 || (parentitem>-1 && itemsbuf[parentitem].family!=itype_book))
        {
            ...
            if(clk==94)
            {
                ...
                if((parentitem<0 || !(itemsbuf[parentitem].flags & ITEM_FLAG2)) &&
                   (itemsbuf[parentitem].flags & ITEM_FLAG5) &&
        ...
        else
        {
            ...
            if(clk==80)
            {
                ...
                if((itemsbuf[parentitem].flags & ITEM_FLAG5) && (Lwpns.idCount(wFire) + Ewpns.idCount(ewFlame))==1)
                {
                    isLit=false;
                    checkLightSources();
                }
        ...
        // Killed by script?
        if(dead==0 && (itemsbuf[parentitem].flags & ITEM_FLAG5 && !(itemsbuf[parentitem].family == itype_candle && itemsbuf[parentitem].flags & ITEM_FLAG2)) && (Lwpns.idCount(wFire) + Ewpns.idCount(ewFlame))==1)
        {
            isLit=false;
            checkLightSources();
        }
