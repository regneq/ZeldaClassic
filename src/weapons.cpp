...
bool weapon::animate(int index, byte lwpns = 0)
{
	switch (lwpns)
	{
		case 1: return LWanimate(index,lwpns);
		default: return EWanimate(index,lwpns);
	}
}

bool weapon::EWanimate(int index, int flags)
{
...
//	Z_scripterrlog("Weapon script is: %d\n",weaponscript);
	...
	case wScript1:
	    ...
	    {
		//Z_scripterrlog("Script LWeapon Type (%d) has a weapon script of: %d\n", id, weaponscript);
		if ( doscript ) ZScriptVersion::RunScript(SCRIPT_EWPN, weaponscript, getUID());
    ...
    case wBait:
    {
	    ...
	    if ( doscript )
	    {
            ...
            dead=23;
	    goto lw_skip_second_bait_script;
        }
...
bool weapon::LWanimate(int index, int flags)
{
    // do special timing stuff
    bool hooked=false;
//	Z_scripterrlog("Weapon script is: %d\n",weaponscript);
